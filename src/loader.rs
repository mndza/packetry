...
impl Loader {
    pub fn open(path: PathBuf) -> Result<Loader, PacketryError> {
    ...
    pub fn next(&mut self) -> Option<Result<(RawPcapPacket, u64), PacketryError>> {
        match self.pcap.next_raw_packet() {
            None => None,
            Some(Err(e)) => Some(Err(PacketryError::from(e))),
            Some(Ok(packet)) => {
                let raw_timestamp =
